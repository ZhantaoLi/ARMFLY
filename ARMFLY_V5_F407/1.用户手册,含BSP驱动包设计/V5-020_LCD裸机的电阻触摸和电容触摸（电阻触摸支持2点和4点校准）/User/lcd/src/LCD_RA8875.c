...
*		V3.1	2016-01-30 armfly  
*					(1) 增加圆角方形输入函数，可用于绘制按钮的圆角。RA8875_FillRoundRect
*					(2) 增加函数 RA8875_BeginBusy() 和 RA8875_EndBusy() 函数
*					(3) 增加位图按钮 BMP_BUTTON_T, 以及相关的函数
*					(4) 增加文字背景通透设置函数: RA8875_SetTextTransp
*		V3.2	2016-04-24	armfly
*					(1) 降低RA8875 内部PLL频率到 68MHz，提高稳定性，可以适应更多类型的显示屏
*					(2) 解决RA8875_DrawRoundRect函数第一次绘制为矩形，原因：内部未设置圆角属性。
*							
...
__IO uint8_t s_ucRA8875Busy = 0;			/* 忙标志, 其他模块通过 RA8875_IsBusy()函数判断忙 */
...
static void RA8875_BeginBusy(void);
static void RA8875_EndBusy(void);
...
void RA8875_WriteReg(uint8_t _ucRegAddr, uint8_t _ucRegValue)
{
	...
	RA8875_BeginBusy();				/* 设置忙标志，避免触摸中断中的程序访问冲突 */

	RA8875_WriteCmd(_ucRegAddr);
	RA8875_WriteData(_ucRegValue);

	//s_ucRA8875Busy = 0;
	RA8875_EndBusy();	/* 此函数会处理电阻屏触摸扫描 */
}
...
/*
*********************************************************************************************************
*	函 数 名: RA8875_BeginBusy
*	功能说明: 设置RA8875忙标志，避免触摸中断访问冲突
*	形    参: 无
*	返 回 值: 1 表示忙； 0 表示空闲
*********************************************************************************************************
*/
...
/*
*********************************************************************************************************
*	函 数 名: RA8875_EndBusy
*	功能说明: 清零RA8875忙标志，避免触摸中断访问冲突
*	形    参: 无
*	返 回 值: 无
*********************************************************************************************************
*/
static void RA8875_EndBusy(void)
{
	...
	if (g_tTP.fReqReadTouch == 1)
	{
		/* 使用fReqReadTouch标志避免递归调用 */
		g_tTP.fReqReadTouch = 2;
		TOUCH_Scan();
		g_tTP.fReqReadTouch = 0;	
	}
}
...
void RA8875_SetTextZoom(uint8_t _ucHSize, uint8_t _ucVSize)
{
	...
	*/
	...
	if (g_LcdDirection > 1)	/* 竖屏  */
	{
		s_reg_22H =  (1 << 4) | ((_ucHSize & 0x03) << 2) | ( _ucVSize & 0x03) | (t << 6);
		RA8875_WriteReg(0x22, s_reg_22H);
	}
	else	/* 横屏 */
	{
		s_reg_22H =  (0 << 4) | ((_ucHSize & 0x03) << 2) | ( _ucVSize & 0x03) | (t << 6);
		RA8875_WriteReg(0x22, s_reg_22H);		
	}
}
