...
*		V1.9	2015-12-21 armfly 解决FT电容触摸芯片ID识别错误问题，需要延迟50ms。修改TOUCH_InitHard()函数。
*		V2.0	2016-01-30 armfly 增加 fReqReadTouch 标志，在主程序空闲时读一次触摸，可以快速响应触笔事件。
*		V2.1	2016-06-28 armfly 解决竖屏g_tParam.TouchDirection = 2校准参数横屏不能用的问题
*
...
/*
    安富莱STM32开发板 + 4.3寸或7寸显示模块（内置RA8875芯片)
...
/* TP_INT 定义GPIO端口 */
#define TP_INT_GPIO_CLK_ENABLE()	__HAL_RCC_GPIOI_CLK_ENABLE()
...
{	
	...
	{		
		...
		case CT_RA8875:		    /* 电阻的 */
			RA8875_TouchInit();  //bsp_DetectLcdType() 内部已经执行初始化 
...
*********************************************************************************************************
*	函 数 名: TOUCH_PenInt
*	功能说明: 判断触摸按下. INT 为低表示有触摸按下。 注意GT811\GT911\GT928在持续按下时，INT会周期性恢复为高电平
*			  脉冲宽度约2ms.
*	形    参: 无
*	返 回 值: 0 表示无触笔按下， 1表示有触摸按下
*********************************************************************************************************
*/
...
void TOUCH_Scan(void)
{
	...
	if (g_GT811.Enable == 1)	/* 7寸电容管屏 GT811 */
	{
		GT811_Timer1ms();	/* 电容触摸屏程序计数器 */
		return;
	}

	if (g_GT911.Enable == 1)	/* 7寸电容管屏 GT911 */
	{
		GT911_Timer1ms();	/* 电容触摸屏程序计数器 */
		return;
	}		
	
	if (g_tFT5X06.Enable == 1)	/* 4.3 5.0寸电容管屏 FT5X06 */
	{
