...
*	文件名称 : main.c
*	版    本 : V1.0
*	说    明 : 定时器PWM输出，支持TIM1-TIM14所有定时器的所有通道。
*              实验目的：
*                1. 学习定时器PWM输出。
*              实验内容：
*                1、系统上电后驱动了1个软件定时器，每100ms翻转一次LED2，同时PB6输出1KHz方波，占空
*                   比50% 。
*              实验操作：
...
*	修改记录 :
*		版本号   日期         作者        说明
*		V1.0    2019-11-02   Eric2013     1. CMSIS软包版本 V5.4.0
*                                         2. HAL库版本 V2.4.0
*
...
/* 定义例程名和例程发布日期 */
#define EXAMPLE_NAME	"V5-定时器PWM输出"
...
{
	printf("操作提示:\r\n");
	printf("1. K1键按下，PB6输出1KHz方波，占空比50%\r\n");
	printf("2. K2键按下，PB6输出10KHz方波，占空比50%\r\n");
	printf("3. K3键按下，PB6输出100KHz方波，占空比50%\r\n");
}
