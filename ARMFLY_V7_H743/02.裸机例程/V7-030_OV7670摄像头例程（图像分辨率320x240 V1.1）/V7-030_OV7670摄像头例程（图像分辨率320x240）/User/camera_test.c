...
*	文件名称 : camera_test.c
*	版    本 : V1.0
*	说    明 : OV7670摄像头例程（图像分辨率320x240）。
*              实验目的：
*                1. 学习OV7670摄像头例程（图像分辨率320x240）。
*              实验内容：
*                1、OV7670摄像头输出320*240分辨率的RGB565颜色格式图像。
*                2、默认上电后，摄像头就是打开的。
...
void TestCamera(void)
{
	...
	while (1)
	{
		...
		if (g_tCam.CaptureOk == 1)
		{
			...
			_LCD_DrawCamera16bpp(PHOTO_X, PHOTO_Y, (uint16_t *)SDRAM_APP_BUF, PHOTO_W, PHOTO_H, 0);
