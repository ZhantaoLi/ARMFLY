...
*	文件名称 : main.c
*	版    本 : V1.0
*	说    明 : 高分辨率定时器HRTIM的PWM实现。
*              实验目的：
*                1. 高分辨率定时器HRTIM的PWM实现。
*              实验内容：
...
*	修改记录 :
*		版本号   日期         作者        说明
*		V1.0    2020-02-01   Eric2013     1. CMSIS软包版本 V5.6.0
*                                         2. HAL库版本 V1.7.0
*
...
/* 定义例程名和例程发布日期 */
#define EXAMPLE_NAME	"V7-HRTIM的PWM输出实现"
#define EXAMPLE_DATE	"2020-02-01"
#define DEMO_VER		"1.0"
...
static void PrintfHelp(void)
{
	...
	printf("2. 配置HRTIM的TIMER D输出两路PWM，周期都是100KHz，PA11引脚输出占空比百分之50 PA12引脚输出的占空比百分之25 \r\n");
}
