...
//*********************************************************************************************************
//*/
//#define TIMx_UP_DMA_STREAM           DMA1_Stream0
//#define TIMx_UP_DMA_REQUEST          DMA_REQUEST_TIM8_UP
//#define TIMx_UP_IRQn                 DMA1_Stream0_IRQn
//#define TIMx_UP_IRQHandler           DMA1_Stream0_IRQHandler

//#define AD7606_BASE    0x60003000


//static DMA_HandleTypeDef TIMDMA;

//TIM_HandleTypeDef  TimHandle = {0};

//int16_t g_sAd7606Buf[16];

//#include "SEGGER_RTT.h"
//	
//void AD7606_DmaCplCb(DMA_HandleTypeDef *hdma)
//{
//	SEGGER_RTT_Write(1, &g_sAd7606Buf[8], 2);
//}
...
//	TIMDMA.Init.FIFOMode            = DMA_FIFOMODE_ENABLE;    /* 禁止FIFO*/
//	TIMDMA.Init.FIFOThreshold       = DMA_FIFO_THRESHOLD_FULL; /* 禁止FIFO此位不起作用，用于设置阀值 */
//	TIMDMA.Init.MemBurst            = DMA_MBURST_INC8;	    /* 禁止FIFO此位不起作用，用于存储器突发 */
//	TIMDMA.Init.PeriphBurst         = DMA_PBURST_INC8;	    /* 禁止FIFO此位不起作用，用于外设突发 */
//	TIMDMA.Init.Request             = TIMx_UP_DMA_REQUEST;     /* 请求类型 */  
//	TIMDMA.Init.Direction           = DMA_PERIPH_TO_MEMORY; //DMA_PERIPH_TO_MEMORY;    /* 传输方向是从存储器到外设 */  
//	TIMDMA.Init.PeriphInc           = DMA_PINC_DISABLE;        /* 外设地址自增禁止 */ 
//	TIMDMA.Init.MemInc              = DMA_MINC_ENABLE;         /* 存储器地址自增使能 */  
//	TIMDMA.Init.PeriphDataAlignment = DMA_PDATAALIGN_HALFWORD;     /* 外设数据传输位宽选择字节，即8bit */ 
//	TIMDMA.Init.MemDataAlignment    = DMA_MDATAALIGN_HALFWORD;     /* 存储器数据传输位宽选择字节，即8bit */    
//	TIMDMA.Init.Mode                = DMA_CIRCULAR; // DMA_NORMAL DMA_CIRCULAR;              /* 正常模式 */
...
//	_htim = (void *)&TimHandle;
//	
//	/* 关联DMA句柄到SPI */
//	__HAL_LINKDMA(_htim, hdma[TIM_DMA_ID_UPDATE], TIMDMA);	
//	
//	/* 配置DMA发送中断 */
