...
*	文件名称 : main.c
*	版    本 : V1.0
*	说    明 : 定时器TIM2，TIM3，TIM4和TIM5的四个比较捕获通道实现微秒级单次延迟。
*              实验目的：
*                1. 学习定时器实现微秒级单次延迟。
*              实验内容：
...
*              实验操作：
*                1. K1键按下，实现一个5微秒的单次延迟，开启后翻转FMC扩展引脚23，时间到后翻转LED4，
*                   再翻转扩展引脚23。
*                2. K2键按下，实现一个10微秒的单次延迟，开启后翻转FMC扩展引脚23，时间到后翻转LED4，
*                   再翻转扩展引脚23。
*                3. K3键按下，实现一个100微秒的单次延迟，开启后翻转FMC扩展引脚23，时间到后翻转LED4，
*                   再翻转扩展引脚23。
*              注意事项：
...
/* 定义例程名和例程发布日期 */
#define EXAMPLE_NAME	"V7-定时器实现微妙级单次延迟"
...
{
	printf("操作提示:\r\n");
	printf("1. K1键按下，实现一个5微秒的单次延迟，开启后翻转FMC扩展引脚23，\r\n   时间到后翻转LED4，再翻转扩展引脚23\r\n");
	printf("2. K2键按下，实现一个10微秒的单次延迟，开启后翻转FMC扩展引脚23，\r\n   时间到后翻转LED4，再翻转扩展引脚2\r\n");
	printf("3. K3键按下，实现一个100微秒的单次延迟，开启后翻转FMC扩展引脚23，\r\n   时间到后翻转LED4，再翻转扩展引脚23\r\n");
}
