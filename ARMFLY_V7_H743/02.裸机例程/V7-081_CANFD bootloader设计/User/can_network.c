...
*	修改记录 :
*		版本号  日期       作者    说明
*		v1.0    2021-01-23 armfly  ST固件库V3.5.0版本。
*
...
{	
	printf("=====%d\r\n",g_Can1RxHeader.DataLength >> 16);
	
	
	
	if ((g_Can1RxHeader.DataLength >> 16) == 64)
	{
...
{
	if ((g_Can2RxHeader.DataLength >> 16) == 64)
	{
		if (g_Can2RxData[0] == 0x01)
		{
			if (g_Can2RxData[1] == 0)
			{
				bsp_LedOff(g_Can2RxData[2]);	/* 关闭LED */
			}
			else
			{
				bsp_LedOn(g_Can2RxData[2]);	/* 打开LED */	
			}
		}
	}
}
