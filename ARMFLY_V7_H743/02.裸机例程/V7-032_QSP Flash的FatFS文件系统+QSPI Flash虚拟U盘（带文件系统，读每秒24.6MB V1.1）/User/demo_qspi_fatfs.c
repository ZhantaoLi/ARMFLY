...
void DemoFatFS(void)
{
	...
	/* 注册SD卡驱动 */
	FATFS_LinkDriver(&SPIFlash_Driver, DiskPath);
...
{
	printf("\r\n------------------------------------------------\r\n");
	printf("首次使用必须使用命令0进行格式化！！！，请选择操作命令:\r\n");
	printf("0 - SPI Flash文件系统格式化\r\n");
	printf("1 - 显示SPI Flash卡根目录下的文件列表\r\n");
...
static void FileFormat(void)
{
	...
	/* 卸载文件系统 */
	result  = f_mount(NULL, DiskPath, 0);
	if (result != FR_OK)
	{
		printf("卸载文件系统失败 (%s)\r\n", FR_Table[result]);
	}
