...
*	文件名称 : main.c
*	版    本 : V1.0
*	说    明 : 定时器PWM输出，支持TIM1-TIM17所有定时器的所有通道。
*              实验目的：
*                1. 学习定时器PWM输出。
*              实验内容：
*                1、系统上电后驱动了1个软件定时器，每100ms翻转一次LED2，同时PB3和PB15输出1KHz方波，占空
*                   比50% 。
...
/* 定义例程名和例程发布日期 */
#define EXAMPLE_NAME	"V7-定时器PWM输出"
...
int main(void)
{
	...
	bsp_SetTIMOutPWM(GPIOB, GPIO_PIN_1,  TIM3,  4, 1000, 5000); /* PB1硬件输出1KHz方波，占空比50% */
...
{
	printf("操作提示:\r\n");
	printf("1. K1键按下，PB1和PB15输出1KHz方波，占空比50%\r\n");
	printf("2. K2键按下，PB1和PB15输出10KHz方波，占空比50%\r\n");
	printf("3. K3键按下，PB1和PB15输出100KHz方波，占空比50%\r\n");
}
