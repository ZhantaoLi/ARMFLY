...
*                   者PLCK。
*                3. 本例子利用LPTIM的超时功能，配置8秒后唤醒停机模式。
*                4、板子正常运行时LED2闪烁，进入停机模式后，LED2停止闪烁。
*                5、进入LPTIM的超时中断后，LED4翻转，方便查看是否正常执行。
*              实验操作：
*                1. K1键按下，进入停机模式, 8秒后自动唤醒。
*              注意事项：
...
/* 定义例程名和例程发布日期 */
#define EXAMPLE_NAME	"V7-低功耗定时器超时模式的停机唤醒"
