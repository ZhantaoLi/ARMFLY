...
#if 0
	#define printf_gt911dbg printf
#else
	#define printf_gt911dbg(...)
#endif
...
static uint8_t GT911_ReadReg(uint16_t _usRegAddr, uint8_t *_pRegBuf, uint8_t _ucLen);
...
void GT911_OnePiontScan(void)
{
	...
	res = GT911_ReadReg(GT911_READ_XY_REG, buf, 1);
	
	if(res == 0)
	{
		printf_gt911dbg("1.failed\r\n");
		return;
	}
	...
	//if (buf[0] == 0)
	{
		GT911_WriteReg(GT911_READ_XY_REG, &clear_flag,	1);
	...
	/* 读取第一个触摸点0 */
	res = GT911_ReadReg(GT911_READ_XY_REG + 1, &buf[1], 5);
	GT911_WriteReg(GT911_READ_XY_REG, &clear_flag,	1);	
	
	if(res == 0)
	{
		printf_gt911dbg("2.failed\r\n");
		return;
	}
	/*
...
#if 0
	printf("%5d,%5d,%5d\r\n",  g_GT911.X0, g_GT911.Y0, g_GT911.P0);
#endif	
}
