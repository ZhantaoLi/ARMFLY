...
*	文件名称 : main.c
*	版    本 : V1.0
*	说    明 : 基于Systick滴答定时器的多组软件定时器实现。
*              实验目的：
*                1. 学习基于Systick滴答定时器的多组软件定时器实现。
*              实验内容：
*                1、启动自动重装软件定时器0，每100ms翻转一次LED1。
*                2、启动自动重装软件定时器1，每100ms翻转一次LED2。
*              实验操作：
*                1、K1键按下，启动软件定时2，单次模式，定时0.5s时间到后，翻转LED3。
*                2、K2键按下，启动软件定时3，单次模式，定时1s时间到后，翻转LED4。
*              注意事项：
...
/* 定义例程名和例程发布日期 */
#define EXAMPLE_NAME	"V7-基于Systick滴答定时器的多组软件定时器实现"
...
{
	printf("操作提示:\r\n");
	printf("1. 启动自动重装软件定时器0，每100ms翻转一次LED1\r\n");
	printf("2. 启动自动重装软件定时器0，每100ms翻转一次LED2\r\n");
	printf("3. K1键按下，启动软件定时2，单次模式，定时0.5s时间到后，翻转LED3\r\n");	
	printf("4. K2键按下，启动软件定时3，单次模式，定时1s时间到后，翻转LED4\r\n");	
}
