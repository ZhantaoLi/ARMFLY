...
extern FATFS usbfs;
extern char USBDISKPath[4];  /* USB逻辑驱动路径，比盘符0，就是"0:/" */
...
static const char * FR_Table[]= 
{
	...
	"FR_MKFS_ABORTED：由于参数错误f_mkusbfs()被终止",	         /* (14) The f_mkusbfs() aborted due to any parameter error */
...
static void ViewRootDir(void)
{
	...
	FILINFO fno;
 	...
 	/* 挂载文件系统 */
	result = f_mount(&usbfs, USBDISKPath, 0);	/* Mount a logical drive */
	if (result != FR_OK)
	{
		printf("挂载文件系统失败 (%s)\r\n", FR_Table[result]);
	}
	...
	/* 打开根文件夹 */
	result = f_opendir(&DirInf, USBDISKPath); /* 如果不带参数，则从当前目录开始 */
	if (result != FR_OK)
	{
		printf("打开根目录失败  (%s)\r\n", FR_Table[result]);
		return;
...
static void CreateNewFile(void)
{
	...
	/* 打开文件 */
	sprintf(path, "%sarmfly.txt", USBDISKPath);
...
static void CreateDir(void)
{
	...
	/* 创建目录/Dir1 */
	sprintf(path, "%sDir1", USBDISKPath);
	result = f_mkdir(path);
	if (result == FR_OK)
	{
	...
	/* 创建目录/Dir2 */
	sprintf(path, "%sDir2", USBDISKPath);
	result = f_mkdir(path);
	if (result == FR_OK)
	{
	...
	sprintf(path, "%sDir1/Dir1_1", USBDISKPath);
	result = f_mkdir(path); /* */
	if (result == FR_OK)
	{
...
static void DeleteDirFile(void)
{
	...
	for (i = 0; i < 20; i++)
	{
		sprintf(path, "%sSpeed%02d.txt", USBDISKPath, i);/* 每写1次，序号递增 */	
		result = f_unlink(path);
		if (result == FR_OK)
		{
			printf("删除文件%s成功\r\n", path);
...
static void WriteFileTest(void)
{
	...
	/* 打开文件 */
	sprintf(path, "%sSpeed%02d.txt", USBDISKPath, s_ucTestSn++); /* 每写1次，序号递增 */	
	result = f_open(&file, path, FA_CREATE_ALWAYS | FA_WRITE);
